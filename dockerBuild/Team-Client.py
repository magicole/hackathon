...
# Global Variables
team_name = 'FooNauts'                        # The Name of the Team
team_auth = '1234'                                  # The Team Authentication Tocken
server_url = 'http://192.241.229.212:81/api'   # URL of the SERVER API
server_ws = 'ws://192.241.229.212:81/ws'       # URL of the Sensors Websocket
...
def team_strategy(parsed_json):
  ...
  """
    ...
    sensor_data = parsed_json['readings']
    # Find this team
    for team in teams_list:
        if team['name'] == team_name:
            if  team['energy'] > 90 or sensor_data['radiation'] > 750 or team['life'] < 10 or (sensor_data['radiation'] > 430 and sensor_data['temperature'] < -40):
          	...
          	print("\nTeam: {0} Action: Shield DOWN!| Energy: {1}".format(team_name, str(team['energy'])))
